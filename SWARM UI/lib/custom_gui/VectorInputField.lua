...
local Widget = require "lib.gui.Widget"
...
function TextField:init(root,length,vector)
    -- TODO: Add auto-completion
    expect(1, root, "table")
    expect(2, length, "number")
    expect(3, vector, "table")
...
function TextField:setVector(vector)
    self.vector = vector
    self.dirty = true
end

-- Gets the text within the TextField
function TextField:getVector()
    return self.vector
end
